...
export default function NonProductionAudit() {
  ...
  const fetchAudits = async () => {
    ...
    try {
      ...
      const response = await api.get("/non-production-audits", {
        params: {
          month,
          year
        }
  ...
  const handleExportExcel = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `non-production-audit-report-${year}-${month}.xlsx`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this audit?")) {
      try {
        await api.delete(`/non-production-audits/${id}`)
        notifications.show({
          title: "Success",
    ...
    <Card shadow="sm" padding="xl" radius="md" withBorder>
      <Group justify="space-between" mb="xl">
        <Group>
          ...
          <Title>Non-Production Audits</Title>
        </Group>
        <Button 
          leftSection={<IconPlus size={16} />} 
          onClick={() => navigate("/6S/non-production-audit/create")}
          size="md"
          color="teal"
        >
          Create New Audit
        ...
        <Grid.Col span={4}>
          <Paper p="md" withBorder radius="md" shadow="xs">
            <Group position="apart">
              ...
              <Badge size="xl" radius="md" color="teal">
      ...
      <Paper withBorder p="md" radius="md" shadow="xs">
        <Group mb="md" position="apart">
          ...
          <Group>
            <Text size="sm" color="dimmed">
              ...
              <Badge color="teal" size="sm">2.5-3.5</Badge> <Badge color="green" size="sm">{">3.5"}</Badge>
            </Text>
        ...
        {loading ? (
          ...
          <Center p="xl">
            <Box ta="center">
              <IconBuildingSkyscraper size={48} color="gray" />
                        ...
                        <Group position="center" spacing="xs">
                          <Tooltip label="Edit Audit">
                            <ActionIcon
                              variant="filled"
                              color="blue"
                              onClick={() => navigate(`/6S/non-production-audit/edit/${audit.id}`)}
                              size="lg"
                            >
                              <IconEdit size={16} />
                            </ActionIcon>
                          ...
                          <Tooltip label="Download Report">
                            <ActionIcon
                              variant="filled"
                              color="green"
                              onClick={() => navigate(`/6S/non-production-audit/preview/${audit.id}`)}
                              size="lg"
                            >
